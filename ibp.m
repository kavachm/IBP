...
clearvars
close all
img_lr=imread('D:\aviris\subsetted data scaled prisma\band15.tif');     % add file path of the input low resolution image that has to be super-resolved. The input must be a single band in tiff format.
ratio=2;                                                                % scaling ratio
sigma = 0.6;                                                            % blur kernel width
iternum = 100;                                                          % number of iterations
breport = true;
tic;                                                                    % starting the calculation of processing time
IntAlg ='bicubic';
img_lri= imresize(img_lr,ratio,IntAlg);                                 % interpolation to target spatial resolution, dependent on the value entered for scaling ratio
[h_hr,w_hr] = size(img_lri);
[h_lr] = size(img_lr,1);
criterion_stop = h_hr*w_hr*0.9999;                                      % stopping criterion  
img_hr=img_lri;                                                         % interpolated output considered for initialization of the iterative process
for i=1:iternum                                                         % beginning of the three phase iterative process
...
end                                                                                    % end of the three phase iterative process
img_bp=img_hr2;
img_sr1=img_bp;
TIME=toc;                                                                              % ending the calculation of processing time              
img_sr=single(img_sr1);                                                             
t1=Tiff('D:\EO1H1490442002308110PZ_1T\outputs\New folder (4) 0.1-2 prisma ibp\band15_2-1_2.tif','w');   % creating a Tiff object for storing the high resolution output generated by IBP algorithm 
...
t1.write(img_sr);
t1.close();
figure;imagesc(imread('D:\EO1H1490442002308110PZ_1T\outputs\New folder (4) 0.1-2 prisma ibp\band15_2-1_2.tif'));  % visualization of the high resolution output 
